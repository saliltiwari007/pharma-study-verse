...
async function scores(request,env){
  ...
  const mode=['beginner','medium','expert'].includes(body?.mode)?body.mode:null;
  const completed=body?.completed===true;
  if(!completed||!mode)return json({error:'Only completed full challenges can enter the shared leaderboard.'},400);
  ...
  if(rawCorrect!==BOARD_TARGETS||rawWrong<0||rawWrong>200||rawScore<0||rawScore>BOARD_SCORE_MAX)return json({error:'Invalid full-challenge result.'},400);
  const expectedScore=Math.max(0,BOARD_SCORE_MAX-(rawWrong*5));if(rawScore!==expectedScore)return json({error:'Score does not match the challenge scoring rules.'},400);
  const name=normaliseName(body?.name);
  await env.DB.prepare(`INSERT INTO game_scores(name,score,mode,correct,wrong,completed,created_at) VALUES(?,?,?,?,?,?,datetime('now'))`).bind(name,rawScore,mode,rawCorrect,rawWrong,1).run();
